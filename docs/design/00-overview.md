...
MERLIN-V is a small, verifiable, in-kernel JIT virtual machine whose
...
A JIT step exists in both eBPF and MERLIN-V — verified bytecode does not
...
- **MERLIN-V's bytecode is RISC-V.** On a RISC-V host or accelerator, the
  ...
  On non-RISC-V hosts (x86\_64, arm64, …) MERLIN-V still needs a real
...
The same MERLIN-V image is loadable into Linux, into RISC-V-based
...
MERLIN-V's hypothesis is that reusing RISC-V eliminates (1) entirely,
...
A MERLIN-V object is an ELF containing standard RISC-V machine code, MERLIN BTF
type info, relocations, a map descriptor table, and a small MERLIN-V meta
section. No new opcode space, no custom register file.

### G3. 1:1 hardware-native execution on RISC-V targets
Both eBPF and MERLIN-V need a JIT step — verified code does not run as
bytecode. The difference is what that step has to do.

On a RISC-V execution target (host CPU, MCU, NIC core, CXL
accelerator) the MERLIN-V "JIT" is a pass-through: verify, relocate,
...
The translation cost is therefore eliminated for MERLIN-V on the
...
project-defined attributes and a `libmerlin`-equivalent runtime. No
project-maintained compiler backend.

### G6. Upstreamability
The Linux kernel components target `bpf-next` and `net-next` and follow
the standard kernel process. The in-tree path is `kernel/merlin/` (TBD,
see [03-kernel-interfaces.md](03-kernel-interfaces.md)).

### NG1. Drop-in eBPF replacement
MERLIN-V is *not* binary-compatible with eBPF. It is a parallel facility
...
Like eBPF, only a restricted dialect of C compiles to a valid MERLIN-V
...
- **Loader trust:** A privileged user-space process (`CAP_BPF` analog,
  TBD `CAP_MERLIN`) submits a signed or capability-gated object. The
...
- **Execution privilege:** In-kernel programs run with kernel privilege
  ...
  (the "MERLIN-V monitor", see [07-jit-and-offload.md](07-jit-and-offload.md)).
...
- **MERLIN-V profile** — the subset of RISC-V instructions, CSRs, and
  memory orderings that are permitted in a MERLIN-V program.
- **MERLIN BTF** — type information format for MERLIN-V; conceptually BTF
  extended with RISC-V-specific relocation kinds. (See
  [04-toolchain.md](04-toolchain.md).)
- **CO-RE-V** — "Compile Once, Run Everywhere" for MERLIN-V: type-relative
  relocations against the running kernel/firmware.
- **MERLIN-V monitor** — minimal trusted runtime on an accelerator core
  that enforces the MERLIN-V sandbox without a full Linux kernel.
- **Host JIT** — the translator that emits non-RISC-V host code from a
  verified MERLIN-V image (only used on x86\_64, arm64, etc.).
...
- Memory model on `merlin-linux-rv64`: default to RVWMO and require
...
- Vector (RVV 1.0): per-program flag inside `merlin-linux-rv64`
  post-RFC, or a new profile name (`merlin-linux-rv64v`)? Out of
  scope for RFC v1.
- `A`-extension AMOs in `merlin-linux-rv64`: allow any
...
- ~~Which RISC-V profile do we pin?~~ → two project profiles,
  `merlin-linux-rv64` and `merlin-rtos-rv32`. See
...
> full ANSI / GNU C, can a MERLIN-V VM run arbitrary applications
...
The C-dialect restriction in eBPF (and in MERLIN-V's default in-kernel
...
### 7.3 Where MERLIN-V already has more headroom than eBPF

Even within the Linux in-kernel constraints, MERLIN-V can comfortably
support more than eBPF does today:

- Larger programs (eBPF caps at ~1M instructions post-verification).
- Larger stacks (eBPF: 512 bytes; MERLIN-V default profile aim: 8–64
...
`fuse-bpf`). MERLIN-V can start from that direction explicitly.

### 7.4 Where the answer flips to "yes"

There are two execution contexts in which MERLIN-V legitimately *is* an
...
1. **RISC-V SmartNICs, DPUs, PCIe / CXL / UALink accelerators.**
   ...
   MERLIN-V program.** This is the offload story made concrete.
2. **Zephyr / FreeRTOS / bare metal on MCUs.** No MMU, cooperative
   or simple preemptive scheduling, single address space anyway.
   "Kernel vs application" is largely vocabulary. MERLIN-V on an MCU
...
```
  ...
  Linux host       eBPF     MERLIN-V (Linux default)   MERLIN-V (sleepable, large)
  kernel:         |--------|----------------------|------ still NOT an app platform

  NIC / DPU                            MERLIN-V (offload profile)
  firmware:                            |----------------- IS the app runtime

  Zephyr / MCU                                          MERLIN-V (RTOS profile)
                                                        |------ IS the app runtime
```
...
### 7.6 The optional third tier — user-mode MERLIN-V

A third execution context, not in the current design plan but worth
recording here: run the MERLIN-V VM in a user-space sandbox process
...
gVisor each do in their own ways. The MERLIN-V twist is that the *same*
...
The clearest MERLIN-V wins are on RISC-V execution targets, where the
...
host, where MERLIN-V still needs a real translating JIT, what does
MERLIN-V offer over eBPF?** This section answers honestly.

### 8.1 What we are NOT claiming

We are **not** claiming that MERLIN-V is faster than eBPF on non-RISC-V
...
first MERLIN-V kernel drop *adds* `kernel/merlin/` alongside `kernel/bpf/`.
...
features, attributes, sanitisers, debug-info, and LTO. MERLIN-V
...
A MERLIN-V program loaded on an x86\_64 host is therefore compiled by
...
**B. One wire format across all targets.** A single `.merlin.o` is
...
JIT. MERLIN-V's image needs no such re-translation on a RISC-V NIC.
...
for the eBPF ISA. If MERLIN-V ships its verifier with the abstract
...
choices. MERLIN-V's profile-parameterised verifier (see §7 of this
...
On any host, MERLIN-V starts at the destination.

**E. Standard developer ergonomics.** `objdump -Mriscv -d` works
out of the box on a MERLIN-V program. GDB can step through it under
QEMU. `perf annotate` on a JITed MERLIN-V program shows familiar
RISC-V; `perf annotate` on eBPF shows host code with no mapping
back to bytecode. Modest individually; accumulates.

**F. ABI compatibility with C calling conventions.** MERLIN-V uses the
...
stabilise in GCC and Clang, MERLIN-V programs that opt into them (via
...
a MERLIN-V verifier or JIT for a class using the RISC-V Instruction
...
> MERLIN-V's primary value proposition is architectural and
