...
1. Loads a MERLIN-V program into your Lab 08 kernel module.
...
- A new ioctl in `merlin.ko` from Lab 08:
  `MERLIN_IOC_ATTACH_VETH(prog_id, ifindex)`.
...
- A user-space tool `merlin-xdpv` that loads a program, attaches it,
  prints counters, and detaches.
- Three demonstrator programs:
  1. `dropall.merlin` — returns `XDP_V_DROP`.
  2. `dropv4.merlin` — returns `DROP` if EtherType==IPv4, else `PASS`.
  3. `count.merlin` — increments a map entry, always returns `PASS`.
...
1. **Verifier time** — wall-clock ns from `MERLIN_IOC_LOAD` start to
   verifier-accepted, measured by user-space `clock_gettime`.
2. **Load-to-run latency** — wall-clock ns from `MERLIN_IOC_LOAD`
   start to the first packet hitting the program.
3. **Per-packet cost** — measure with a 64-byte UDP storm into the
   `veth` peer, comparing:
   - Baseline (no MERLIN-V attached).
   - Empty MERLIN-V (`return PASS`).
   - The demonstrator.
...
3. **Background** — eBPF and MERLIN-V, ≤ 1 page.
...
- `kmod/merlin.ko` extended with `MERLIN_IOC_ATTACH_VETH`.
- `user/merlin-xdpv/` CLI.
...
- Demonstrate a MERLIN-V program built with **Clang** and one built with
...
- A working user-space and in-kernel MERLIN-V stack.
